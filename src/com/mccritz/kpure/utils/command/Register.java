...
public class Register
{
	...
	public Register()
	{
		...
		try
		{
			this.commandMap = (SimpleCommandMap) Bukkit.getServer().getClass().getDeclaredMethod("getCommandMap", new Class[0])
	...
	public BaseCommand constructFromAnnotation(final IBaseCommand base)
	{
		try
		{
			...
			if (execute.isAnnotationPresent(BaseCommandAnn.class))
			{
				...
				BaseCommand command = new BaseCommand(commandAnn.name(), commandAnn.permission() == null ? null : commandAnn.permission(),
						commandAnn.commandUsage(), commandAnn.aliases())
				{
					@Override
	...
	private ArrayList<Class<?>> getClassesInPackage(final String pkgname)
	{
		...
		final CodeSource codeSource = kPure.getInstance().getClass().getProtectionDomain().getCodeSource();
	...
	public void registerCommand(final String cmd, final BaseCommand executor) throws Exception
	{
		...
		if (command == null)
		{
			...
			command = (PluginCommand) constructor.newInstance(cmd, kPure.getInstance());
		}
