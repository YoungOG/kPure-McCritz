...
public class Profile
{
	...
	public void loadProfileData(@Nullable ProfileRequest<Profile> callback, boolean username)
	{
		try
		{
			Document document = pCollection.find(Filters.eq(username ? "currentName" : "uniqueID",
					username ? Pattern.compile("^" + currentName + "$", Pattern.CASE_INSENSITIVE) : uniqueID.toString())).first();
			...
			{
				permanentBans
						.add(new PermanentBan(doc.getString("punisherUUID") != null ? UUID.fromString(doc.getString("punisherUUID")) : null,
			...
			{
				temporaryBans
						.add(new TemporaryBan(doc.getString("punisherUUID") != null ? UUID.fromString(doc.getString("punisherUUID")) : null,
			...
			{
				permanentMutes.add(
						new PermanentMute(doc.getString("punisherUUID") != null ? UUID.fromString(doc.getString("punisherUUID")) : null,
								doc.getString("reason"), doc.getString("dateCreated"), doc.getBoolean("isActive")));
			}
			...
			{
				temporaryMutes.add(
						new TemporaryMute(doc.getString("punisherUUID") != null ? UUID.fromString(doc.getString("punisherUUID")) : null,
								doc.getLong("length"), doc.getString("reason"), doc.getString("dateCreated")));
			}
	...
	{
		return isPermanentlyBanned() || isTemporarilyBanned() || kPure.getInstance().getPunishmentManager().isIPBanned(getCurrentIP());
	}
	...
	public void lookup(CommandSender sender)
	{
		...
		if (hasElevatedPermission)
		{
			MessageManager.sendMessage(sender,
					"&7Past IPs(&c" + getIpList().size() + "&7): &c" + getIpList().toString().replace("[", "").replace("]", ""));
		}
		
		MessageManager.sendMessage(sender,
				"&7Past Names&7(&c" + getNameList().size() + "&7): &c" + getNameList().toString().replace("[", "").replace("]", ""));
