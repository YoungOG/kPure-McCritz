...
public class CommandClearMobs extends BaseCommand {
    ...
    public void execute(CommandSender sender, String[] args) {
        for (World w : Bukkit.getWorlds()) {
            MessageManager.sendMessage(sender, "&7World " + (w.getEnvironment() == World.Environment.NORMAL ? "&a" + w.getName() : "&c" + w.getName()) + " &7removed &c" + w.getEntitiesByClass(Monster.class).size() + " &7monsters.");
